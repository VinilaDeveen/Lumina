...
export default function PhEligibilityTestForAppointment() {
  // Alert 
  ...
  const submitEligibilityTest = async () => {
    ...
    try {
      const response = await axios.post(`http://localhost:8080/api/lumina/eligibilityTest/appointment?appointmentId=${appointmentId}`,answersObj, config);
      if (response.data === true) {
        statusCode.current = 200;
        errorMessage.current = "Eligible to donate blood";
        setTimeout(() => {
          navigate(`/phlebotomist/donationsProcess/${userId}/${appointmentId}`);
        }, 3000);
